...
import { Download, Filter, Search, Clock } from "lucide-react"
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@/components/ui/table"

interface TimeLogsPanelProps {
...
export default function TimeLogsPanel({ simulationEngine, isRunning }: TimeLogsPanelProps) {
  ...
  useEffect(() => {
    ...
    const updateInterval = setInterval(() => {
      try {
        const logData = simulationEngine.getTimeLogs()
  ...
  useEffect(() => {
    if (autoScroll && tableRef.current && isRunning) {
  ...
  const exportLogs = () => {
    ...
    try {
      // Create CSV header
      const headers = ["Timestamp", "Depth (ft)", "Event Type", "Parameters"]
      ...
      a.href = url
      a.download = `drilling_time_logs_${new Date().toISOString().split("T")[0]}.csv`
    ...
    <Card className="h-full">
      <CardHeader className="pb-3">
        <div className="flex justify-between items-center">
          <CardTitle>Time-Based Logs</CardTitle>
      ...
      <CardContent>
        ...
        <div ref={tableRef} className="border rounded-md h-[calc(100vh-300px)] overflow-auto">
          <Table>
            ...
            <TableBody>
              {filteredLogs.length > 0 ? (
                filteredLogs.map((log, index) => (
                  <TableRow key={index} className={index === filteredLogs.length - 1 && isRunning ? "bg-blue-50" : ""}>
